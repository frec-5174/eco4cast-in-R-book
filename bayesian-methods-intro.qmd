...
If you have multiple parameters your MCMC chain, you randomly draw `posterior_sample_indices` and use values for all the parameters at that iteration. By doing this you are representing the joint distribution of the parameter - e.g., if parameter 1 is high, parameter 2 is always low. If you select `posterior_sample_indices` for each parameter, you break the correlations of parameters that the MCMC method estimated and, as a result, overestimate the uncertainty.
...
In a ecosystem example, we often use net fluxes (like net ecosystem exchange) to calibrate models. Since NEE is the net of respiration and photosynthesis, the same NEE can emerge from a system with large values for respiration and photosynthes as a system with small values for respiration and photosynthesis.  Therefore the parameters that govern respiration and photosynthesis are not identifiable using only NEE data for parameter estimation.   

Identifiablity issues can be addressed by adding additional data sources for parameter estimation.  For example, in the next chapter we use NEE and LAI in the calibration.  The LAI data helps separate out the low production from the high production system because ...

Identifiablity issues can be addressed by using "stronger" priors that have distributions with less spread. These informed priors can come from data synthesis, surveying experts, or measurements. In some cases they are so informed that you assume they are a fixed value.  Justification for the parameter value choice when fixing a parameter or using a strong prior.   
### Data leakage
...
You can use data that you will use in your likelihood calculate to involve the starting point of your MCMC chain.  In some cases, a maximum likelihood fit of your model, like in Chapter XXX, will yield starting points that are very close mean of the posterior (just without the uncertainty estimation).  By starting your MCMC chain at this maximum likelihood estimates for the parameters, you will reduce the burn-in iterations and increase the computationally efficiency of your Bayesian analysis.  This is not data leakage.
