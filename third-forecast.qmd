...
In the third forecast generation assignment, you will automate the submission of your forecast. By running your forecast code each day, you will update your model with the latest data, which is a simple form of data assimilation.
...
You will be generating forecasts of water temperature at NEON lake sites.
...
You will use a Docker container (`eco4cast/neon4cast-rocker`) to ensure a consistent computation environment each day. Docker is based on a rocker container that already has R, RStudio, and many common packages (e.g., tidyverse). It also contains packages that are specific to the NEON Forecasting Challenge.
...
Go to <https://docs.docker.com/get-docker/> to install the relevant version for your platform (available for PC, Mac, and Linux). Also see <https://docs.docker.com/desktop/>.
...
5.  You should see an R Studio interface with all the packages pre-installed and ready to go.
...
1.  Update your repository to include the GitHub Action file provided in the Automation tutorial listed above.  Save it at `.github/workflows/run_forecast.yaml`.
2.  Change the file at the end of the GitHub Action file to be `forecast_code_template_mod6.R` rather than `forecast_code_template.R`
3.  Commit and push updated files to GitHub
...
Thomas, R.Q. and C. Boettiger. 2026. Cyberinfrastructure to Support Ecological Forecasting Challenges. Accepted at *Ecosphere*. Pre-print available at ESS Open Archive. <http://doi.org/10.22541/essoar.175917344.44115142/v2>.
