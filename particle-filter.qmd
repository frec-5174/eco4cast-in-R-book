...
The `sd_init` sets the initial uncertainty in the model states (you could use the distribution from an observation with observation uncertainty added)
...
for(t in 2:num_time_steps){
  ...
  return(list(x = x, x_prior = x_prior))
}
}
```
...
#| label: fig-parts-alone
 ...
 tibble(time = 1:num_time_steps,
  ...
  geom_line()
  labs(x = "Date", y = "log(Flu cases)", title = "State of Virginia") +
  theme_bw()
```
...
output <- particle_filter(x) #function from previous chunk
x <- output$x
```
...
combined %>% 
  ...
  labs(x = "Date", y = "log(flu cases)", title = "Google Flu Trends for Virginia") +
  theme_bw()
```
...
output <- particle_filter(x)
x <- output$x
x_prior <- output$x_prior
...
prior_hist <- tibble(x = density(df1$x)$y*0.3+i,
              y = density(df1$x)$x,
              type = "prior (forecast)",
              date = i)
...
post_hist <- tibble(x = density(df2$x)$y*0.3+i,
              y = density(df2$x)$x,
              type = "posterior (analysis)",
              date = i)

obs <- rnorm(1000, y[i], sd = sd_obs)
obs_hist <- tibble(x = density(obs)$y*0.3 + i,
              y = density(obs)$x,
...
combined |> 
  ...
  geom_point(size = 0.4) + 
  labs(x = "Day", y = "log(flu cases)") +
  theme_bw()
```

### PF with parameter estimation

The estimate of parameters using PF is also straightforward. Here we will estimate the `b1` parameter in Dynamic Linear Model (DLM). The DLM uses minimum temperature as a covariate
...
The values for `b0`, `b2`, and `sd_add` are the mean values from the prior model calibration.
...
b1 <- array(NA, dim = c(num_time_steps, num_particles))
