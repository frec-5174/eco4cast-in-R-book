...
-   The carbon in tree leaves is represented by data measuring leaf area index (which is related to leave carbon through a mass-to-area conversion).
...
In this section, we will be calculating carbon in live and dead trees at a NEON site. The carbon in live trees represents the wood carbon stock in @fig-process-model and the dead trees represent a component of the soil organic matter stock in @fig-process-model. At the end we will have a *site-level* mean carbon stock in *live trees* and *dead trees*for each year that was sampled from the plots that represent ecosystem under the flux tower (e.g., tower plots). We use the tower plots so that it corresponds to same ecosystem as the NEON `nee` data.
...
Soil carbon data in contained in two NEON data products: one that describes the physical characteristics of the soil (depth and density) and another the describes the carbon concentration of the soil. Ultimately multiplying the density by the carbon concentration gives the total carbon.
...
-   Determines the height of the horizon (`biogeoBottomDepth - biogeoTopDepth`) and coverts to total mass of soil in the horizon using the bulk density.
...
Next we will combine our estimates of carbon in each component for visualization and to aggregated into the wood and som stocks below.
...
Combine pools of carbon together to match the stocks used in our simple process model. - wood = live trees (stem and coarse roots) + fine roots - som = dead trees + down wood + soil carbon
...
Leaf area index can be used a proxy for leaf carbon. The forest model convert leaf carbon into LAI using a leaf mass to area parameter. As a result, we can use leaf area index (LAI) from the MODIS satellite sensor to constrain and evaluate LAI predictions. MODIS LAI product is a 8-day mean for a 500m grid cell.
...
NEE flux data is used to help constrain the net of the photosynthesis and respiration in the simple forest model. It is already processed for used in the NEON Ecological Forecasting Challenge. Here we read in that data.
