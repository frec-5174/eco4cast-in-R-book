...
What is the overarching goal of this chapter and why is it placed here in the sequence? Consider whether it would come better just before the NEON-EFI challenge tutorial? (In that case maybe consider adding in videos on the aquatic program - OR leave it here but put the section 18.1 video + the aquatic program video in Ch. 8 with the tutorial?)

This chapter relates to the previous chapter by.....\[need to fill in\]

This chapter will relate to the next chapter by.... \[need to fill in\]
...
In this document, we will be creating a carbon budget for a NEON site. <!--# insert text or list describing learning objectives and chapter roadmap here --> This activity was developed in collaboration with John Smith at Montana State University.
...
Short paragraph on the purpose of forest carbon budgets and how one creates them at a high level, paired with a simple conceptual figure.
...
The elevation, latitude, and longitude are needed to convert the tree diameter measurements to biomass <!--# this is why I think you need a short paragraph introducing carbon budgets - this is the first mention of tree diameter measurements! --> and are found on the NEON page describing the site.

```{r}
site <- "OSBS"
elevation <- 46 #166	what are these commented out numbers?
...
In this section, we will be calculating carbon in live and dead trees at a NEON site <!--# this is where you could refer back to a conceptual carbon budget figure to help students understand the states you are estimating -->. At the end we will have a *site-level* mean carbon stock in *live trees* for each year that was sampled from the plots that represent ecosystem under the flux tower (e.g., tower plots). We use the tower plots so that it corresponds to same ecosystem as the NEON `nee` data.
...
The species names in the mapping and tagging table need to be separated into the genus and species so that we can calculate the biomass <!--# is that because you calculate biomass by genus? need to clarify -->.
...
Tidy up the individual tree data to include only live trees from the tower plots. Also create a variable that is the year of the sample date. We will filter the data based on measurement heights of 130 cm to only include data that had diameter at breast height (dbh) measurements. <!--# I didn't know that 130 cm was the official height for dbh so this was confusing at first - maybe mention this above in new carbon budget overview section and define dbh? -->
...
To calculate the biomass of each tree in the table, we will use the `get_biomass` function from the `allodb` package, which is a package that.... <!--# what is the allodb package?  --> This function takes as arguments: `dbh`, `genus`, `species`, and `coords`. We have already extracted genus and species and filtered to dbh measurements.
...
<!--# I can't run the code past here because the allodb package is not available for R 4.3.1. this package needs to be added to install.R I think -->
...
Soil carbon data in contained in two NEON data products: product A and product B. <!--# briefly describe products here -->
...
Next we will combine our estimates of carbon in each fraction... <!--# in order to calculate the total carbon stored in the ecosystem? -->
...
Combine pools of carbon together to match the stocks used in our simple process model. This converts it to a long data format. <!--# Now, we will be able to use the data to....? (how does this relate back to process model) -->
...
<!--# this came out of nowhere a bit - can you set up that you need this at the beginning of the chapter when you explain how this chapter relates to preceding and subsequent chapters? -->We can use leaf area index (LAI) from the MODIS satellite sensor to constrain and evaluate LAI predictions. MODIS LAI product is a 8-day mean for a 500m grid cell.
...
<!--# same comment as for MODIS - can you introduce the need/use of these data up front? -->Learn about flux data used to calibrate and evaluate NEE fluxes in the model
...
<!--# Now, we have a complete carbon budget file that is stored in a format compatible with our simple forest process model. This will allow us to do XX, YY, and ZZ. -->Save the observations to a csv file for use in later chapters.
