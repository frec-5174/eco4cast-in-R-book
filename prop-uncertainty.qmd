...
To answer the question you will calculate the variance across ensemble members for each uncertainty type for each date.  This table can be plotted to visually compare the magnitude of each source of uncertainty. 
...
Using the code above as a guide, create code to estimate uncertainty based on the propagation all sources at the same time (unlike the one-at-a-time approach above). Plot the forecast with the combined uncertainty.
